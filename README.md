...
evidence and supports MDCROP/PROOF publisher inputs without making FLETCH
...
captures a shareable local query URL. Selecting a row records a stable row deep
link, and **Copy row link** shares that selected detail. Loaded source previews
...
view that is preserved in source-preview deep links, **Copy source link** for the
exact loaded preview URL, and a compact JSON outline when the loaded data is JSON.
The same behavior is available from `/api/source?matched_only=true`. For followed
...
status, graph, and tips bundled for MDCROP indexing, PROOF rendering, dashboards,
...
entries while the mock adapter owns the MDCROP/MDPORT/PROOF interpretation.
...
- [`docs/specs/consumer-adapter-scout.md`](docs/specs/consumer-adapter-scout.md)
  maps ICELINES, apportionment/BISECT, ROUTE, MDCROP, MDPATH, and PROOF to
