...
- **documents**: a generated PROOF/MDCROP/Markdown/backend artifact describes a
...
derived from MDCROP index rows. PROOF backends can render Markdown, HTML, or other
...
`fletch.publisher-bundle.v1` summarizes publisher inputs for MDCROP/PROOF backends:
MDCROP row counts, PROOF document counts, local URL counts, and optional quiver or
...
Tips give MDCROP, PROOF, CLIs, and adapters a cheap way to decide what data is
...
members so MDCROP and PROOF can index portable bundles without reading cache
objects.
...
adapters, MDCROP/PROOF generated files, or checked-in configs can hand FLETCH a
...
Ledger entries should remain safe to publish through MDCROP/PROOF. They should
...
- `cache summary`: aggregate status rows into cache health counts and expected
  versus actual byte totals for CI, MDCROP, and PROOF status views.
...
Machine-readable publish scout for MDCROP, PROOF, dashboards, and local status
