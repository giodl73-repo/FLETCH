...
fn registry_web_serves_summary_search_detail_and_html() -> Result<(), Box<dyn Error>> {
    let index_path = write_test_index()?;
    ...
    let search = http_get(address, "/api/search?text=storm&limit=10")?;
    ...
    assert!(detail.contains("fixtures/seed-storm.json"));
...
fn write_test_index() -> Result<std::path::PathBuf, Box<dyn Error>> {
  ...
  "rows": [
    {
      ...
      "node_kind": "document",
      "source_urls": ["fixtures/seed-storm.json"],
      "source_kinds": ["file"],
