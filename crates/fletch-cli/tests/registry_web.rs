...
fn registry_web_serves_summary_search_detail_and_html() -> Result<(), Box<dyn Error>> {
    ...
    assert!(html.contains("selected_matched_only"));
    ...
    let linked_html = http_get(
        address,
        "/?text=storm%20seed&sort=fletch_id&direction=desc&offset=1&limit=25&selected_registry_id=storm-foundation-assets&selected_fletch_id=storm.foundation.seed-storm&selected_source=0&selected_line_start=1&selected_matched_only=true",
    )?;
