...
pub(crate) fn fetch_registry_url(client: &reqwest::blocking::Client, url: &str) -> Result<FletchRegistry> {
    let text = client
        .get(url)
        .send()?
        .error_for_status()?
...
pub(crate) fn write_json<T: serde::Serialize + ?Sized>(value: &T, output: Option<PathBuf>) -> Result<()> {
