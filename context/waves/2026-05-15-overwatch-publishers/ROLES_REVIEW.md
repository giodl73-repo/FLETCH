...
| Provenance Auditor | MDCROP, MDLOOM, URL, and publisher bundle rows retain source schema references back to authoritative machine contracts. | Pass |
...
| Performance Engineer | Reports summarize existing rows and read-only MDCROP/MDLOOM/URL outputs now support bounded slices for large local publisher surfaces. | Pass |
| MDLOOM/MDCROP Publisher | Overwatch directly provides index rows, document anchors, URL maps, and bundle summaries for local rendering/indexing. | Pass |
...
| Contract Checker | Foundation spec and code consistently name `fletch.mdcrop-index.v1`, `fletch.mdloom-docs.v1`, `fletch.local-url-map.v1`, and `fletch.publisher-bundle.v1`. | Pass |
...
| F-I3 Data Integration | MDCROP/MDLOOM outputs remain backend-neutral and adapter-friendly. | Pass |
