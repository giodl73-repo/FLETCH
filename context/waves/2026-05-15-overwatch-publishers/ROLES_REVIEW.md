...
| Provenance Auditor | CROP, MDLOOM, URL, and publisher bundle rows retain source schema references back to authoritative machine contracts. | Pass |
...
| Performance Engineer | Reports summarize existing rows and read-only CROP/MDLOOM/URL outputs now support bounded slices for large local publisher surfaces. | Pass |
| MDLOOM/CROP Publisher | Overwatch directly provides index rows, document anchors, URL maps, and bundle summaries for local rendering/indexing. | Pass |
...
| Contract Checker | Foundation spec and code consistently name `fletch.crop-index.v1`, `fletch.mdloom-docs.v1`, `fletch.local-url-map.v1`, and `fletch.publisher-bundle.v1`. | Pass |
...
| CROP Indexer | `fletch.crop-index.v1` provides a direct local corpus index input. | Pass |
| MDLOOM Publisher | MDLOOM document and URL contracts support multiple renderer/backends without core coupling. | Pass |
...
| F-I3 Data Integration | CROP/MDLOOM outputs remain backend-neutral and adapter-friendly. | Pass |
...
- Future MDLOOM backends should keep generated Markdown/HTML linked to these
