...
Expose MDCROP/MDLOOM-friendly publisher contracts that render or index FLETCH
...
- `fletch-core`: MDCROP index, MDLOOM document manifest, local URL map, and
...
| 02 | MDLOOM document manifest | done | Added `fletch.mdloom-docs.v1` document anchors over MDCROP index rows. |
| 03 | Local URL map | done | Added `fletch.local-url-map.v1` stable local URLs over MDLOOM document anchors. |
| 04 | Publisher bundle report | done | Added `fletch.publisher-bundle.v1` summary over MDCROP, MDLOOM, URL, quiver, and adapter views. |
| 05 | Publisher slices | done | Added bounded MDCROP/MDLOOM/URL publisher slices for large local surfaces. |
